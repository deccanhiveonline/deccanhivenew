...
export default function Header() {
  ...
  return (
    <>
          ...
          <div className="flex items-center justify-between h-16 sm:h-20">
            <div className="flex-1 flex justify-start">
              <a href="/" onClick={handleLogoClick} className="flex items-center gap-2 sm:gap-3 group flex-shrink-0 w-fit">
                <div className="w-9 h-9 sm:w-10 sm:h-10 bg-white rounded-xl flex items-center justify-center overflow-hidden group-hover:scale-105 transition-transform shadow-sm border border-white/20">
                  <img src="/logo2.webp" alt="Deccan Hive Logo" className="w-full h-full object-contain p-1" />
                </div>
            ...
            <nav className="hidden lg:flex items-center justify-center">
              <div className="flex items-center gap-1 bg-background/5 backdrop-blur-lg py-1.5 px-1.5 rounded-full border border-border/30">
                {navItems.map((item) => {
                  ...
                  return (
                    <Link key={item.name} href={item.path} className={`relative cursor-pointer text-sm font-medium px-5 py-2 rounded-full transition-colors ${isActive ? 'text-foreground' : 'text-muted-foreground hover:text-foreground'}`}>
                      {item.name}
                      {isActive && (
                        <motion.div layoutId="tubelight" className="absolute inset-0 w-full bg-primary/10 rounded-full -z-10" initial={false} transition={{ type: "spring", stiffness: 350, damping: 30 }}>
            ...
            <div className="flex-1 flex justify-end">
              <div className="flex items-center gap-2 sm:gap-4">
                ...
                <button onClick={() => setIsOpen(!isOpen)} className="lg:hidden w-9 h-9 sm:w-10 sm:h-10 rounded-xl border border-border/50 bg-background/50 backdrop-blur-sm flex items-center justify-center text-foreground hover:bg-secondary transition-colors">
      ...
      {isOpen && <div className="fixed inset-0 bg-background/90 backdrop-blur-md z-[90] lg:hidden" onClick={() => setIsOpen(false)} />}

      <div className={`fixed top-0 right-0 h-full w-full max-w-sm bg-background border-l border-primary/20 transform transition-transform duration-300 ease-out z-[100] lg:hidden ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="flex flex-col h-full relative">
          <div className="flex items-center justify-between p-4 sm:p-6 border-b border-primary/10">
            ...
            <button onClick={() => setIsOpen(false)} className="w-10 h-10 rounded-xl border border-primary/20 bg-primary/5 flex items-center justify-center text-foreground hover:bg-primary/10 transition-colors">
              <X size={20} />
            </button>
          </div>
          <nav className="flex-1 p-4 sm:p-6 space-y-1.5 overflow-y-auto">
            {navItems.map((item, index) => (
              <Link key={item.name} href={item.path} onClick={() => setIsOpen(false)} className={`block px-4 py-3.5 rounded-xl font-medium transition-all duration-200 ${pathname === item.path ? 'bg-primary text-primary-foreground shadow-lg shadow-primary/20' : 'text-foreground hover:bg-primary/10 hover:text-primary'}`} style={{ animationDelay: `${index * 50}ms` }}>
          ...
          <div className="p-4 sm:p-6 border-t border-primary/10 bg-gradient-to-t from-primary/5 to-transparent">
            <Button className="w-full bg-primary text-primary-foreground rounded-xl h-12 text-base font-semibold shadow-lg shadow-primary/20 hover:shadow-primary/30 transition-all" asChild>

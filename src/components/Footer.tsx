          ...
          <div className="sm:col-span-2 lg:col-span-1">
            <a href="/" onClick={handleLogoClick} className="flex items-center space-x-3 mb-5 group w-fit">
              <div className="w-10 h-10 bg-white rounded-lg flex items-center justify-center overflow-hidden group-hover:scale-105 transition-transform shadow-sm border border-border/10">
                <img src="/logo1.webp" alt="Deccan Hive Logo" width="40" height="40" className="w-full h-full object-contain p-1" />
              </div>
              <div>
            ...
            <div className="flex flex-wrap gap-3">
              <a href="https://www.instagram.com/deccanhive/" target="_blank" rel="noopener noreferrer" className="w-10 h-10 bg-muted/50 rounded-full flex items-center justify-center text-muted-foreground hover:bg-primary hover:text-primary-foreground transition-all duration-300"><Instagram size={18} /></a>
              <a href="https://facebook.com/deccanhive" target="_blank" rel="noopener noreferrer" className="w-10 h-10 bg-muted/50 rounded-full flex items-center justify-center text-muted-foreground hover:bg-primary hover:text-primary-foreground transition-all duration-300"><Facebook size={18} /></a>
              <a href="https://linkedin.com/company/deccanhive" target="_blank" rel="noopener noreferrer" className="w-10 h-10 bg-muted/50 rounded-full flex items-center justify-center text-muted-foreground hover:bg-primary hover:text-primary-foreground transition-all duration-300"><Linkedin size={18} /></a>
              <a href="https://youtube.com/@deccanhive" target="_blank" rel="noopener noreferrer" className="w-10 h-10 bg-muted/50 rounded-full flex items-center justify-center text-muted-foreground hover:bg-primary hover:text-primary-foreground transition-all duration-300"><Youtube size={18} /></a>
            </div>
          </div>
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-1 gap-2 sm:gap-3">
              {quickLinks.map((link) => (
                <Link key={link.name} href={link.path} className="text-sm text-muted-foreground hover:text-primary transition-colors">
                  {link.name}
                </Link>
              ))}
            </div>
        ...
        <div className="border-t border-muted-foreground/20 mt-10 lg:mt-12 pt-6 lg:pt-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="flex justify-center sm:justify-end items-center gap-4 sm:gap-6">
              <Link href="/privacy-policy" className="text-xs sm:text-sm text-muted-foreground hover:text-primary transition-colors">Privacy Policy</Link>
              <span className="text-muted-foreground/30">|</span>
              <Link href="/terms-conditions" className="text-xs sm:text-sm text-muted-foreground hover:text-primary transition-colors">Terms & Conditions</Link>
            </div>
          </div>
        </div>
      </div>

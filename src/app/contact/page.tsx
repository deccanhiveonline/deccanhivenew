...
export default function Contact() {
  ...
  return (
    <>
      <BreadcrumbSchema items={[{ name: 'Home', url: 'https://deccanhive.com' }, { name: 'Contact Us', url: 'https://deccanhive.com/contact' }]} />
      ...
      <section className="relative min-h-[40vh] md:min-h-[60vh] flex items-center justify-center px-4 pt-20 md:pt-24 overflow-hidden">
        <motion.div className="container mx-auto text-center max-w-5xl" style={{ y: heroY, opacity: heroOpacity }}>
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: [0.25, 0.1, 0.25, 1] }}>
            ...
            <GradualSpacing text="Get In Touch" className="text-3xl sm:text-4xl lg:text-6xl font-bold text-foreground" duration={0.3} delayMultiple={0.03} />
            <p className="text-muted-foreground text-sm md:text-lg mt-4 md:mt-6 max-w-2xl mx-auto leading-relaxed">Ready to transform your business? Let's discuss how our digital solutions can help you achieve your goals.</p>
          </motion.div>
        </motion.div>
      </section>
            ...
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, ease: "easeOut" }} viewport={{ once: true }} className="premium-card p-5 md:p-8 rounded-2xl">
              ...
              <form onSubmit={handleSubmit} className="space-y-4 md:space-y-6" noValidate>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
                  ...
                  <div>
                    <Input type="email" name="email" placeholder="Your Email" value={formData.email} onChange={handleChange} required maxLength={255} className={`bg-secondary border-border h-11 md:h-12 text-sm md:text-base rounded-xl ${errors.email ? 'border-red-500' : ''}`} />
                    {errors.email && <p className="text-red-500 text-xs mt-1">{errors.email}</p>}
                  </div>
                </div>
                <Input type="text" name="subject" placeholder="Subject" value={formData.subject} onChange={handleChange} maxLength={200} className="bg-secondary border-border h-11 md:h-12 text-sm md:text-base rounded-xl" />
                <div>
                ...
                {submissionStatus && <p className="text-center text-primary font-medium text-sm">{submissionStatus}</p>}
              </form>
            </motion.div>

            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, ease: "easeOut", delay: 0.1 }} viewport={{ once: true }} className="space-y-4 md:space-y-6">
              <div className="premium-card p-5 md:p-8 rounded-2xl">
                <h2 className="text-xl md:text-2xl font-bold mb-4 md:mb-6">Contact <span className="text-gradient-gold">Info</span></h2>
                <div className="space-y-4 md:space-y-6">
                  {contactInfo.map((item, i) => (
                    <div key={i} className="flex items-start gap-3 md:gap-4">
                      <div className="w-10 h-10 md:w-12 md:h-12 bg-primary/10 rounded-xl flex items-center justify-center flex-shrink-0"><item.icon className="w-4 h-4 md:w-5 md:h-5 text-primary" /></div>
                      <div>
              ...
              <div className="premium-card p-4 md:p-6 rounded-2xl">
                ...
                <div className="grid grid-cols-4 md:grid-cols-2 gap-2 md:gap-4">
                  {socialLinks.map((social, i) => (
                    <a key={i} href={social.href} target="_blank" rel="noopener noreferrer" className={`flex flex-col md:flex-row items-center justify-center gap-1 md:gap-2 border rounded-xl p-2 md:p-3 transition-all ${social.color}`}>
      ...
      <section className="py-12 md:py-20 px-4">
        <div className="container mx-auto max-w-7xl">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} viewport={{ once: true }} className="text-center mb-8 md:mb-12">
            <h2 className="text-2xl md:text-4xl font-bold mb-4">Our <span className="text-gradient-gold">Global Reach</span></h2>
            <p className="text-muted-foreground text-sm md:text-base max-w-2xl mx-auto">From Hyderabad to the world - we serve clients across continents with tailored digital solutions.</p>
          </motion.div>
          <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8 }} viewport={{ once: true }} className="premium-card p-4 md:p-8 rounded-2xl">

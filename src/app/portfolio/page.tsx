...
export default function Portfolio() {
  ...
  return (
    <>
      <BreadcrumbSchema items={[{ name: 'Home', url: 'https://deccanhive.com' }, { name: 'Portfolio', url: 'https://deccanhive.com/portfolio' }]} />
      ...
      <section className="relative min-h-[80vh] flex items-center justify-center px-4 pt-24 overflow-hidden">
        <motion.div className="container mx-auto text-center max-w-5xl" style={{ y: heroY, opacity: heroOpacity }}>
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }} className="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-12 max-w-4xl mx-auto">
        ...
        <div className="container mx-auto max-w-7xl">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {portfolioItems.map((item, index) => (
              <motion.div key={index} initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: index * 0.15, ease: "easeOut" }} viewport={{ once: true, margin: "-50px" }} className="premium-card p-8 group">

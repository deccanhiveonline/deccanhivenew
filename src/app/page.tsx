...
export default function Home() {
  ...
  const portfolioItems = [
    { title: 'E-commerce Growth Campaign', client: 'Kumar Electronics', result: '450% ROI achieved' },
    { title: 'B2B Lead Generation', client: 'TechSolutions Pvt Ltd', result: '200+ qualified leads/month' },
  ];

  return (
    <>
      <section className="relative min-h-screen flex items-center justify-center px-4 pt-24 overflow-hidden">
        <motion.div className="container mx-auto max-w-5xl text-center" style={{ y: heroY, opacity: heroOpacity }}>
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, ease: [0.16, 1, 0.3, 1] }}>
            ...
            <Magnetic strength={0.3}>
              <Button size="lg" className="bg-primary text-primary-foreground hover:bg-primary/90 rounded-full px-6 sm:px-8 h-12 sm:h-14 text-base font-semibold" asChild>
                <Link href="/contact">
                  Get Started
      ...
      <motion.section className="py-20 lg:py-32 px-4" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ duration: 0.8 }} viewport={{ once: true }}>
        <div className="container mx-auto max-w-7xl">
          <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} viewport={{ once: true }} className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6 mb-12">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {portfolioItems.map((item, index) => (
              <motion.div key={item.title} initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: index * 0.15 }} viewport={{ once: true }} className="group premium-card p-8">
      ...
      <section className="py-16 lg:py-32 px-4">
        <div className="container mx-auto max-w-5xl">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} viewport={{ once: true }} className="premium-card p-8 sm:p-12 lg:p-20 text-center relative overflow-hidden">
            ...
            <div className="flex justify-center">
              <Magnetic strength={0.3}>
                <Button size="lg" className="bg-primary text-primary-foreground hover:bg-primary/90 rounded-full px-8 sm:px-10 h-12 sm:h-14 text-base font-semibold" asChild>
                  <Link href="/contact">
                    Start Your Project
                    <ArrowRight className="ml-2 h-4 w-4 sm:h-5 sm:w-5" />
                  </Link>
